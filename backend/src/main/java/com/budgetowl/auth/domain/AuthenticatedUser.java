 ...
 */
public record AuthenticatedUser(UUID userId, String email, boolean instanceAdmin)
    ...
    public static AuthenticatedUser of(UserAccount user) {
        return new AuthenticatedUser(user.id(), user.email(), user.isInstanceAdministrator());
    }
